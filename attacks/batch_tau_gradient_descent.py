...
class BatchTauGradientDescent(Attack):
    """
    ...
    def __init__(self):
        ...
        super(BatchTauGradientDescent, self).__init__()
    ...
    def run(self, model, images, objective, writer=None, prefix=''):
        ...
        super(BatchTauGradientDescent, self).run(model, images, objective, writer, prefix)
        ...
        self.taus = torch.from_numpy(numpy.ones(batch_size, dtype=numpy.int) * self.tau)
        """ (numpy.ndarray) Taus per example. """
        ...
        def forward(inputs):
            if self.auxiliary:
                _, logits = model(inputs, auxiliary=self.auxiliary)
            else:
                logits = model(inputs)
            return logits
        ...
        for i in range(self.max_iterations + 1):
            ...
            # 1/
            # Forward pass.
            output_logits = forward(images + self.perturbations)
            ...
            for b in range(batch_size):
                if error[b].item() < success_errors[b]:
                    if successes[b].item() > 0 and self.taus[b] >= 0:  # f(x) != y
                        self.taus[b] -= 1
            ...
            gradient_magnitudes = torch.mean(torch.abs(gradients.view(batch_size, -1)), dim=1) / \
                                  self.perturbations.size()[0]
            ...
            if i == self.max_iterations:
                break
            if torch.all(successes > 0) and torch.all(self.taus < 0):
                break
            ...
            # 7/ Update.
            if self.backtrack:
                next_perturbations = self.perturbations - torch.mul(common.torch.expand_as(self.lrs, self.gradients),
                                                                    self.gradients)
                ...
                next_output_logits = forward(images + next_perturbations)
