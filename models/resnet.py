...
from .resnet_block import ResNetBottleneckBlock, ResNetBlock
...
class ResNet(Classifier):
    ...
    def __init__(self, N_class, resolution=(1, 32, 32), blocks=[3, 3, 3], block='', activation='relu', normalization='bn', channels=64, zero_residuals=True, dropout=False, **kwargs):
        """
        ...
        common.torch.kaiming_normal_(conv1.weight, mode='fan_out', nonlinearity=activation, scale=self.init_scale)
        ...
        for i in range(len(self.blocks)):
            ...
            if stride != 1 or expanded_planes != out_planes * block_class.expansion:
                ...
                common.torch.kaiming_normal_(conv.weight, mode='fan_out', nonlinearity=activation, scale=self.init_scale)
                if self.include_bias:
            ...
            sequence.append(block_class(expanded_planes, out_planes, stride=stride, downsample=downsample, normalization=self.normalization,
                                        init_scale=self.init_scale, include_bias=self.include_bias, zero_residuals=self.zero_residuals, activation=self.activation, dropout=self.dropout))
